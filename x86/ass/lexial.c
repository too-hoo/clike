//
// Created by toohoo on 18/5/2020.
//
// --词法分析
void checkReserved();
/** ********************************************************************************************************************
 ...
 * ********************************************************************************************************************/
/**
 * 扫描器Scanner:从文件中按照顺序读取一个字符返回[文件不读完不要修改fin，否则数据丢失]
 * 返回值：-1表示文件结束
 * 关联：全局变量ch,行号lineNum
 */
 // getChar()
...
int lineNum=0;  // 行号
int line[maxLen]; //
int chAtLine=0; // 当前的字符列的位置
int lineLen=0;  // 当前行的长度
int getChar()
{
    if(chAtLine>=lineLen) // 超出索引，行读完成， >= 防止出现强制读取的bug
    {
 ...
 * 返回值： 解析出的符号，负数表示的是有错误，绝对值表示错误码
 * 关联：全局变量sym,id[], str[], num, letter
 */
// getSum()
int err=0; // 记录错误
enum symbol sym=null; // 当前的符号
char id[idLen+1]; // 存放标识符号
int num=0;  // 存放数字
...
int getSym()
{
    ...
    if(ch>='a'&&ch<='z'||ch>='A'&&ch<='Z'||ch=='_'|| ch=='@'||ch=='.')  // _,@,字母开头的_,@,字母，数字串：标识符（关键字）
    {
        ...
        int reallen=0;  // 实际标识符号的长度
        int f;          // getChar 返回的标记
        // 取出标识符
        do
        {
            reallen++;
            if(idCount<idLen) // 标识符过长的部分略去
            {
                id[idCount]=ch;
                idCount++;
            }
    ...
    else
    {
        ...
        switch(ch)
        {
            ...
            case '"':
                ...
                {
                    reallen++;
                    if(strCount<stringLen)  // 字符串过长部分略去
                    {
                        str[strCount]=ch;
                        strCount++;
                    }
            ...
            case ']':
                ...
                GET_CHAR
                break;
            case 0:     // 不再这个位置到达文件末尾，会暂时不处理，等到下一次调用的时候这里返回
                sym=null;
                return -1;
                break;
            default:
