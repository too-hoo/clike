...
extern char str[]; // 记录当前的string， 给error处理
extern char id[]; // 记录当前的ident
extern int num;  // 记录当前的num
...
struct lb_record;
extern Table table;
extern string curSeg;  // 当前段的名称
extern int dataLen; // 有效数据长度
extern Elf_file obj; // 输出文件
extern int inLen;  //已经输出的指令长度，调试使用
...
int getSym();
void program();
void writeBytes(int value,int len);  // 按照小端顺序输出任意不大于4字节长度的数据
